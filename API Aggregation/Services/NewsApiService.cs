...
namespace API_Aggregation.Services
{
    ...
    public class NewsApiService : INewsApiService
    {
        ...
        public async Task<ArticlesResult> GetNewsAsync(NewsFilter filter)
        {
            try
            {
                ...
                {
                    Q = filter.DefaultTitle ?? "News",
                    SortBy = filter.DefaultSortBy ?? SortBys.Popularity,
                    Language = filter.DefaultLanguage ?? Languages.EN,
                    From = filter.DefaultFromDate ?? DateTime.Today.AddMonths(-1),
                });
